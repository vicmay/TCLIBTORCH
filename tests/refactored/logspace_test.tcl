# tests/refactored/logspace_test.tcl
# Test file for refactored torch::logspace command
...
if {![llength [info commands torch::logspace]]} {
    puts "❌ Command torch::logspace not found after loading libtorchtcl.so"
    exit 1
}

# Configuration
set COMMAND_NAME "logspace"
set COMMAND_OLD "torch::logspace"
set COMMAND_NEW "torch::logspace"
...
# Test 2: Original positional syntax with base
puts "Test 2: Original positional syntax with base..."
if {[catch {
    set tensor2 [$COMMAND_OLD 0 2 3 2.0]
    puts "  Original syntax with base: OK"
} result]} {
    puts "  ❌ Original syntax failed: $result"
    exit 1
}
...
if {[catch {
    set tensor3 [$COMMAND_OLD 0 3 4 10.0 float32 cpu]
...
if {[catch {
    set tensor4 [$COMMAND_NEW -start 0 -end 3 -steps 4]
...
# Test 5: New named parameter syntax - with base
puts "Test 5: New named parameter syntax - with base..."
if {[catch {
    set tensor5 [$COMMAND_NEW -start 0 -end 2 -steps 3 -base 2.0]
    puts "  Named parameters with base: OK"
} result]} {
    puts "  ❌ Named parameters failed: $result"
    exit 1
}
...
if {[catch {
    set tensor6 [$COMMAND_NEW -start 0 -end 3 -steps 4 -base 10.0 -dtype float32 -device cpu]
...
if {[catch {
    set tensor7 [$COMMAND_NEW -dtype float64 -start 1 -end 4 -steps 5 -base 2.0 -device cpu]
...
if {[catch {
    set pos_tensor [$COMMAND_OLD 0 3 4 10.0 float32 cpu]
    set named_tensor [$COMMAND_NEW -start 0 -end 3 -steps 4 -base 10.0 -dtype float32 -device cpu]
...
for {set i 0} {$i < $iterations} {incr i} {
    $COMMAND_OLD 0 3 4 10.0 float32 cpu
}
...
set start [clock clicks -milliseconds]
for {set i 0} {$i < $iterations} {incr i} {
    $COMMAND_NEW -start 0 -end 3 -steps 4 -base 10.0 -dtype float32 -device cpu
}
set end [clock clicks -milliseconds]
...
if {[catch {
    # Create tensor with logspace
    set tensor [$COMMAND_NEW -start 0 -end 2 -steps 5 -base 2.0 -dtype float32]
...
if {[torch::cuda_is_available]} {
    ...
    if {[catch {
        set cuda_tensor [$COMMAND_NEW -start 0 -end 3 -steps 4 -device cuda]
